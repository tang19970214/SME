  ...
  <div>
    ...
    <el-dialog class="dialog-mini" width="50%" :title="listDialogTitle()" :visible.sync="listDialogForm" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
      ...
      <el-form ref="dataForm_basic" :model="list_basic" label-position="right" label-width="120px" v-if="chooseInfo.value == 'basic'">
        ...
        <el-form-item size="small" :label="'感興趣的資訊'">
          <el-select class="filter-item w-full" v-model="list_basic.interestItems" placeholder="請選擇感興趣的資訊" multiple>
          ...
          <el-input v-model="list_basic.interestOther" placeholder="請輸入其他感興趣的資訊" v-if="getInterestOther()"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  computed: {
    ...
    getInterestOther() {
      return () => {
        if (this.list_basic.interestItems?.includes("其他")) {
          return true;
        } else {
          this.list_basic.interestOther = "";
          return false;
        }
      };
    },
